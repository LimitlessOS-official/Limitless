...
static void (*pci_monitor_event)(const char* event, pci_device_t* dev) = NULL;
/* Set PCI monitoring callback */
void hal_pci_set_monitor_callback(void (*callback)(const char* event, pci_device_t* dev)) {
    pci_monitor_event = callback;
}
...
static void pci_scan_function(uint8_t bus, uint8_t slot, uint8_t func) {
    ...
    driver_t* driver = find_best_driver_for_pci_device(dev);
    if (driver) {
    ...
    if (pci_monitor_event) {
        pci_monitor_event("discover", dev);
    }
}
...
/* Initialize PCI subsystem with async hotplug/removal */
status_t hal_pci_init(void) {
    ...
    /* Monitoring/statistics hook for init */
    if (pci_monitor_event) {
        pci_monitor_event("init", NULL);
    }
    return STATUS_OK;
}

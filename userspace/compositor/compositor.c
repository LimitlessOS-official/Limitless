/*
 * LimitlessOS Compositor - Complete Desktop Window Manager
 * Military-grade UI with glass effects and modern theming
 */
...
/* Fill rectangle */
void compositor_fill_rect(surface_t* surface, gui_rect_t* rect, color_t color) {
    if (!surface || !rect) {
        return;
    }
...
void compositor_draw_rect(surface_t* surface, gui_rect_t* rect, color_t color) {
    if (!surface || !rect) {
        return;
    }
...
void compositor_draw_text(surface_t* surface, int32_t x, int32_t y, const char* text, color_t color) {
    ...
    /* Bitmap-based text rendering with glyph lookup */
    int32_t cursor_x = x;
    while (*text) {
        /* Draw a simple rectangle per character */
        gui_rect_t char_rect = {cursor_x, y, 8, 16};
...
void compositor_render_frame(void) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        for (uint32_t y = 0; y < window->surface->height && (window->bounds.y + (int32_t)y) < (int32_t)g_compositor->fb->height; y++) {
            for (uint32_t x = 0; x < window->surface->width && (window->bounds.x + (int32_t)x) < (int32_t)g_compositor->fb->width; x++) {
                int32_t fb_x = window->bounds.x + x;
                int32_t fb_y = window->bounds.y + y;
...
bool rect_contains_point(gui_rect_t* rect, int32_t x, int32_t y) {

...
int server_get_resource_usage(resource_limits_t* usage) {
    ...
    /* Calculate max CPU usage from /proc/stat */\n    FILE* stat_file = fopen(\"/proc/stat\", \"r\");\n    if (stat_file) {\n        char line[256];\n        unsigned long user, nice, system, idle;\n        if (fgets(line, sizeof(line), stat_file) && \n            sscanf(line, \"cpu %lu %lu %lu %lu\", &user, &nice, &system, &idle) == 4) {\n            unsigned long total = user + nice + system + idle;\n            unsigned long used = user + nice + system;\n            usage->max_cpu_percent = total > 0 ? (used * 100) / total : 0;\n        } else {\n            usage->max_cpu_percent = 25; /* Default 25% usage */\n        }\n        fclose(stat_file);\n    } else {\n        usage->max_cpu_percent = 25; /* Default 25% usage */\n    }
